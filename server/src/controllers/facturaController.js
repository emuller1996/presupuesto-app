const { Factura, Proyecto, Presupuesto, Contracto } = require("../db")
...
const createFactura = async (req, res) => {
    ...
    try {
        ...
        if(parseInt(data.montoTotal) > contracto.monto_disponible){
            return res.status(422).json({message :`el contracto no tiene fondos para la factura, tiene disponible ${contracto.monto_disponible}`})
        }
