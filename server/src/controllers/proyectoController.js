...
const getProyectoById = async (req, res) => {
  try {
    ...
    const dataAdd = {
      gastoPorcentaje:
        (proyectoById.montoUsado / proyectoById.montoTotal) * 100,
      restantePorcenjate:
        (proyectoById.montoDisponible / proyectoById.montoTotal) * 100,
      asignadoPorcentaje:
        (proyectoById.montoAsignado / proyectoById.montoTotal) * 100,
    };
...
const getContractosByProyecto = async (req, res) => {
  try {
    const contractos = await Contracto.findAll({
...
const getFacturasByProyecto = async (req, res) => {
  ...
  try {
    const facturas = await Factura.findAll({
      include: [{ model: Contracto, where: { ProyectoId: idProyecto } }],
    });
  ...
  try {
    ...
    if (data.monto_total > proyecto.montoDisponible) {
      return res.status(422).json({
        message: `El proyecto no tiene fondos para el contracto, solo tiene disponible ${proyecto.montoDisponible}`,
      });
    }
...
const updateProyectoById = async (req, res) => {
  try {
    ...
    data.montoDisponible =
      data.montoTotal -
      (pro.dataValues.montoAsignado + pro.dataValues.montoUsado);

    if (data.montoTotal < pro.dataValues.montoAsignado) {
      return res.status(422).json({
        message: `el proyecto ya tiene asignado ${pro.dataValues.montoAsignado}, no se puede permite esta accion.`,
      });
    }
    ...
    if (presu.totalRestante < 0) {
      return res.status(422).json({
        message: `El presuesto no tiene fondos para actualizar el proyecto.`,
      });
    }
    ...
    return res
      .status(202)
      .json({ message: "si se actualizo correctamente.", proyecto: p });
  } catch (error) {
    console.log(error);
  }
};
